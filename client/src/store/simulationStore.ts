...
interface OrbitState {
  ...
  cameraAction: {
    type: "zoomIn" | "zoomOut" | "reset" | "toggleRotate";
    id: number;
  } | null;
  triggerCamera: (
    action: "zoomIn" | "zoomOut" | "reset" | "toggleRotate",
  ) => void;
  ...
  // Real-time Tracking
  hoverCoords: [number, number, number] | null;
...
export const useStore = create<OrbitState>((set, get) => ({
  ...
  triggerGltfDownload: () =>
    set((state) => ({ downloadGltfTrigger: state.downloadGltfTrigger + 1 })),
  downloadCsvTrigger: 0,
  triggerCsvDownload: () =>
    set((state) => ({ downloadCsvTrigger: state.downloadCsvTrigger + 1 })),

  showAnalysisPanels: false,
  toggleAnalysisPanels: () =>
    set((state) => ({ showAnalysisPanels: !state.showAnalysisPanels })),

  logs: [
    {
      id: "init",
      msg: "Gala Studio System Initialized",
      type: "info",
  ...
  cameraAction: null,
  triggerCamera: (action) =>
    set(() => ({ cameraAction: { type: action, id: Date.now() } })),

  hoverCoords: null,
  mouseScreenPos: null,
  setHoverState: (coords, screenPos) =>
    set({ hoverCoords: coords, mouseScreenPos: screenPos }),
  ...
  setUnits: (units) => {
    ...
    if (units === "solarsystem" && state.potentialType === "milkyway") {
      ...
      get().addLog(
        `Switched to Kepler potential for Solar System units.`,
        "warn",
      );
    } else {
    ...
    set((state) => ({
      potentialParams: { ...state.potentialParams, ...defaults },
    }));
  ...
  setPotentialType: (type) => {
    ...
    if (type === "milkyway" && state.units === "solarsystem") {
      ...
      get().addLog(
        `Switched to Galactic units for Milky Way potential.`,
        "warn",
      );
      return;
    }
  ...
  integrateOrbit: async (silent = false) => {
    ...
    console.log("[SimulationStore] 🚀 Starting Integration", {
      potentialType,
      units,
      integrator,
    });
    ...
    if (potentialType === "kepler") {
      try {
        ...
        if (!silent) {
          get().addLog(
            `Kepler prediction completed (Local Browser)`,
            "success",
          );
        }
        return;
        ...
        console.warn(
          "Client-side integration failed, falling back to server",
          e,
        );
      }
    }
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        if (!silent) {
          get().addLog(
            isCloudMode
              ? `Orbital ensemble computed (100 variants)`
              : `Numerical integration finished (${data.points.length} points)`,
            "success",
          );
        }
    ...
    try {
      ...
      if (data.status === "success") {
        set({
          ...
          successMsg: data.isChaotic
            ? "Chaos Detected!"
            : "Orbit appears stable.",
        });
        get().addLog(
          `Dynamical analysis complete. ${data.isChaotic ? "Chaos detected" : "System appears stable"}.`,
          data.isChaotic ? "warn" : "success",
        );
        setTimeout(() => set({ successMsg: null }), 3000);
      }
    } catch (err) {
